...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            # update statistics
            if not isinstance(data['rgb_images'], list) and data['rgb_images'] is not None:
                batch_size = data['rgb_images'].shape[loader.stack_dim]
            elif not isinstance(data['ir_images'], list) and data['ir_images'] is not None:
                batch_size = data['ir_images'].shape[loader.stack_dim]
            elif not isinstance(data['depth_images'], list) and data['depth_images'] is not None:
                batch_size = data['depth_images'].shape[loader.stack_dim]
        ...
        if i % self.settings.print_interval == 0 or i == loader.__len__():
            ...
            for name, val in self.stats[loader.name].items():
                if (self.settings.print_stats is None or name in self.settings.print_stats):
                    if hasattr(val, 'avg'):
                        if "Top" in name:
                            print_str += '%s: %.2f[CP:%d, TP:%d]  ,  ' % (
                                name, val.avg, int(val.avg / 100.0 * val.count * get_world_size()),
                                val.count * get_world_size())
                        else:
